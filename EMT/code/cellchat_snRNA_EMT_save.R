...
df <- readRDS(here("final_before/RDS/snRNA.RDS"))

df <- subset(df, subset = cell_type_submain %in% c("Fibroblasts", "AT1", "AT2"))
...
saveRDS(cellchat_control, here("EMT/RDS/cellchat_snRNA_EMT_control.RDS"))

cellchat_covid <- aggregateNet(cellchat_covid)
saveRDS(cellchat_control, here("EMT/RDS/cellchat_snRNA_EMT_covid.RDS"))

cellchat_ipf <- aggregateNet(cellchat_ipf)
saveRDS(cellchat_control, here("EMT/RDS/cellchat_snRNA_EMT_ipf.RDS"))
