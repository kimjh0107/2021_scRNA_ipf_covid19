...
```{r}
DimPlot(refquery, reduction = "umap", group.by = "integrated_snn_res.0.8", label = TRUE, label.size = 3, repel = TRUE) + NoLegend()
```


```{r}
DimPlot(refquery, reduction = "umap", group.by = "integrated_snn_res.0.8", label = TRUE, label.size = 3, repel = TRUE) + NoLegend()
#ggsave(here('figure/03_celltype_matching/New_cluster_celltype.pdf')) 
